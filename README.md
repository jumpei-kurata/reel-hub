...
| コメント自動いいね＆絵文字返信 | ✅ 稼働中（**起床時にアプリ内で自動実行**・外部cron不要） |
| トークン自動リフレッシュ | ✅ 稼働中（**起床時に残日数チェック→25日未満で自動更新**・外部cron不要） |
...
| 動画DL | yt-dlp |
| ホスティング | Render 無料枠（スリープ運用・常時起動しない） |
| スリープ対策 | **なし（あえて寝かせる）**。初回アクセスは~60秒のコールドスタート。保守は起床時にアプリ内で自動実行 |
...
### スリープ運用（keep-alive はしない）

本サービスは**あえて常時起動させない**。14分おきの keep-alive ping は Render 無料枠の月間インスタンス時間（750h/ワークスペース）をほぼ使い切りサスペンドを招くため**廃止した**（2026-06）。

代わりに、アプリは**スリープから起床するたび（=アクセスのたび）に `main.py` の lifespan から自動保守をバックグラウンド実行**する（コメント処理＋トークン残日数チェック→必要時のみ更新）。外部スケジューラ（cron-job.org / GitHub Actions）は**一切不要**。初回アクセスは~60秒のコールドスタートを待つ前提。
...
`main.py` の lifespan が**起床ごとに `process_comments()` をバックグラウンド実行**する（外部cron不要）。`POST /api/instagram/process-comments` エンドポイントは手動/デバッグ用に残置。

> /tmp はコールドスタートで揮発するため処理済みキャッシュは毎回リセットされるが、`_already_replied` の API チェックで二重返信は防止される（いいねの再試行は無害）。
...
旧構成では cron-job.org で月1 POST していたが、スリープ運用への移行で**廃止**。更新は起床時の自動保守が担う。`POST /api/refresh-token?secret=...` は手動更新用に残置（下記「手動実行」）。
